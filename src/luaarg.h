...
#define LNIL {LUA_TNIL, {.index=0}}
#define LBOOLEAN(b) {LUA_TBOOLEAN, {.boolean=(b)}}
#define LLIGHTUSERDATA(p) {LUA_TLIGHTUSERDATA, {.userdata=(p)}}
#define LNUMBER(n) {LUA_TNUMBER, {.number=(n)}}
#define LSTRING(s) {LUA_TSTRING, {.string=(s)}}
#define LTABLE {LUA_TTABLE, {.index=0}}
#define LFUNCTION {LUA_TFUNCTION, {.index=0}}
#define LUSERDATA(p) {LUA_TUSERDATA, {.userdata=(p)}}
#define LTHREAD {LUA_TTHREAD, {.index=0}}

#define LNULL {NULL, 0, 0, 0, LNIL}

#define LREQUIRED(key, typeflags, defaultval) {key, 1, typeflags, 0, defaultval}

#define LREQUIREDNIL(key)               LREQUIRED(key, FLAG_NIL, LNIL)
#define LREQUIREDBOOLEAN(key, value)    LREQUIRED(key, FLAG_BOOLEAN, LBOOLEAN(value))
#define LREQUIREDLIGHTUSERDATA(key, value)    LREQUIRED(key, FLAG_LIGHTUSERDATA, LLIGHTUSERDATA(value))
...
#define LREQUIREDUSERDATA(key, value)   LREQUIRED(key, FLAG_USERDATA, LUSERDATA(value))
#define LREQUIREDTHREAD(key)            LREQUIRED(key, FLAG_THREAD, LTHREAD)

#define LOPTIONAL(key, typeflags, defaultval) {key, 0, typeflags, 0, defaultval}

#define LOPTIONALNIL(key)               LOPTIONAL(key, FLAG_NIL, LNIL)
#define LOPTIONALBOOLEAN(key, value)    LOPTIONAL(key, FLAG_BOOLEAN, LBOOLEAN(value))
#define LOPTIONALLIGHTUSERDATA(key, value)    LOPTIONAL(key, FLAG_LIGHTUSERDATA, LLIGHTUSERDATA(value))
...
#define LOPTIONALUSERDATA(key, value)   LOPTIONAL(key, FLAG_USERDATA, LUSERDATA(value))
