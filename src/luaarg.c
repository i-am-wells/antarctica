...
int luaarg_get_value_from_stack(lua_State* L,
                                int idx,
                                int type,
                                luaarg_t* arg) {
  arg->val.type = type;
...
void luaarg_check_type(lua_State* L,
                       int actualtype,
                       const luaarg_t* arg,
                       int argi) {
  ...
  if (!(LUA_TYPE_FLAG(actualtype) & arg->typeflags)) {
    luaL_error(L, "type \"%s\" not allowed for argument #%d (\"%s\")",
               lua_typename(L, actualtype), argi, arg->key);
  }
}
...
int luaarg_find_keyword(lua_State* L, int tableidx, luaarg_t* args) {
  ...
  for (luaarg_t* arg = args; arg->key; arg++) {
    ...
    // If not found in table and not marked found in arg list, if required, give
    // error, else use default
    ...
    if ((actualtype == LUA_TTABLE) || (actualtype == LUA_TTHREAD) ||
...
int luaarg_check_required(lua_State* L, const luaarg_t* args) {
  ...
  for (const luaarg_t* arg = args; arg->key; arg++) {
    if (arg->required && !arg->found) {
      return luaL_error(L, "missing required argument #%d (\"%s\")", argi,
                        arg->key);
    }
    argi++;
  }
